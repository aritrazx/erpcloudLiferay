...
<div class="col-md-12 col-sm-12 col-xs-12 pull-center ">
	...
	if(componentList.size()>0){
	 ...
	 if (earningList.contains(component.getPayType())) {
	  HashMap<Long,String> hashmap = SalaryRegisterDAO.getEmployeeEarningComponent(monthYear, employee.getEmployeeId());
	  if(hashmap.size()>0){
...
%>
	 ...
	 for(PayComponent component:componentList){
		 if(component.getPayType().equalsIgnoreCase(SalaryRegisterPortletKeys.Deduction)) {
		  HashMap<Long,String> hashmap = SalaryRegisterDAO.getEmployeeDeductionComponent(monthYear, employee.getEmployeeId());
		  if(hashmap.size()>0){
			  boolean isComponent = false;
		  for(Map.Entry<Long,String> entry:hashmap.entrySet()){		 
			  if(component.getComponentId()==entry.getKey()){
