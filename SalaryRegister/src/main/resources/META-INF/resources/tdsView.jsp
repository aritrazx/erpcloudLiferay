...
<%@page import="com.liferay.famocom.erpcloud.service.YearSettingLocalServiceUtil"%>
<%@page import="com.liferay.famocom.erpcloud.model.YearSetting"%>
...
<%
	if (themeDisplay.isSignedIn()) {
		...
		if (isViewPermission) {
			...
			printPdf.setParameter("employeeId", empIdStr);
			...
			if (Validator.isNotNull(empIdStr)) {
				if (Validator.isNotNull(monthYear)) {
					try {
						employee = EmployeeLocalServiceUtil.getEmployee(Long.valueOf(empIdStr));
					} catch (Exception e) {
						e.printStackTrace();
					}
					try {
						declarationMonthly = TaxDeclarationMonthlyLocalServiceUtil
								.findTaxDeclarationMonthlyByMonthYearAndEmployeeId(Long.valueOf(empIdStr),
										Long.valueOf(monthYear));
					} catch (Exception exception) {
						exception.printStackTrace();
					}
...
%>
            ...
            <body  style="font-family: serif; font-size: 10pt;">
              ...
              <tr><td colspan="2" align="center"><h4>Tax Computation as on <%=fromdate %> <%=year.subSequence(2, 4) %><!-- '.$fromdate.' --> For the Financial Year <%=financialStartYear %>-<%=financialEndYear %><!--  '.FINANCIALYEAR.'  --></h4></td></tr>
