...
<%
	List<ExpenseEntryHistory> expenseHistoryList = (List<ExpenseEntryHistory>) renderRequest
			.getAttribute("expenseHistoryList");
	if (Validator.isNotNull(expenseHistoryList)) {
		System.out.println("======== expenseHistoryList :-" + expenseHistoryList.toString());
	}

	PortletURL iteratorUrl = renderResponse.createRenderURL();
	iteratorUrl.setParameter("jspPage", "/META-INF/resources/viewForAccApproval.jsp");
	...
	PortletURL myExpAccUpdateURL = renderResponse.createActionURL();
	myExpAccUpdateURL.setParameter("javax.portlet.action", "myExpAccUpdate");
	myExpAccUpdateURL.setParameter("expenseId", expenseId);

	//String strDate = formatter.format(addDate);
%>

<portlet:actionURL name="interviewAdd" var="interviewAddURL" />
<aui:form name="interview" action="<%=myExpAccUpdateURL.toString()%>"
	method="post">
	<aui:row>
		<aui:col span="6">
			</br>
			<aui:row>
				...
				<aui:col span="6">
					<%
						ProjectMaster projectMaster = ProjectMasterLocalServiceUtil
													.getProjectMaster(expEntry.getProjectId());
 ...
 %>
			...
			<aui:input name="approved" type="radio" label="Approved" value="5" />
			<aui:input name="approved" type="radio" label="Reject" value="6" />
...
<%
	if (Validator.isNotNull(expenseHistoryList)) {
%>
<div><%@ include file="history-list.jsp"%>
</div>
<%
	}
%>