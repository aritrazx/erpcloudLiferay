...
<table class="table table-bordered no-margin table-hover">
	...
	<tbody id="custom-table">
		<%
			...
			for (Performance pObj : performanceList) {
				...
				try {
					...
					appPerformObj = AppraisalPerformanceLocalServiceUtil.findByApprIdSettingIdAndEmployeeIdPerformId(
							getMasterObj.getAppraisalId(), setup.getAppraisalSettingId(), employee.getEmployeeId(),
							pObj.getPerformanceId());
					System.out.println("appPerformObj...>>> "+appPerformObj);
					System.out.println("appPerformObj.getPerformanceComm()...>>> "+appPerformObj.getPerformanceComm()+"  appPerformObj.getRatingIdEmp()..>>>  "+appPerformObj.getRatingIdEmp());
					}
					} catch (Exception e) {
				}
		%>
		<tr>
			...
			<td class="text-left col-md-4"><textarea label="" class="form-control"
					name="<portlet:namespace/>performance_<%=pObj.getPerformanceId()%>" value=""
					placeholder="Notes/Comments"  ><%=Validator.isNotNull(appPerformObj)? appPerformObj.getPerformanceComm():"" %></textarea></td>
			<td class="text-left col-md-2"><select label="" class="form-control       "
			 ...
			 <c:forEach items="${manageAppraisalModel.ratingMasterModel}" var="rating">
				<aui:option value="${rating.ratingId}" selected="${rating.ratingId==empDetails.employeetypeId ? true : false }">
						...
						for (RatingMaster rObj : ratingMasterList) {
							...
							if(Validator.isNotNull(appPerformObj)){
								if(rObj.getRatingId()==appPerformObj.getRatingIdEmp()){
									System.out.println("rObj.getRatingId()<<<<...>>> "+rObj.getRatingId()+"...>>appPerformObj.getRatingIdEmp()<<<..."+appPerformObj.getRatingIdEmp());
									selected = "selected"; 
								}else{
									selected = ""; 
								}
					...
					%>
					<option value="<%=rObj.getRatingId()%>" <%=selected%>><%=rObj.getRatingNum()%> - <%=rObj.getRatingKey()%></option>
					<%
						}
					%>
			</select></td>
			<%-- <td class="text-left col-md-2"><select label=""
				name="<portlet:namespace/>performanceManRat_<%=pObj.getPerformanceId()%>" disabled="disabled">
			...
			<option value="${rating.ratingId}" selected="${rating.ratingId==empDetails.employeetypeId ? true : false }">
				<c:out value="${rating.ratingKey} - ${rating.ratingNum}"/></option>
					...
					<%
						for (RatingMaster rObj : ratingMasterList) {
					%>
					<option value="<%=rObj.getRatingId()%>" ><%=rObj.getRatingNum()%> - <%=rObj.getRatingKey()%></option>
					<%
						}
					%>
			</select></td> --%>
